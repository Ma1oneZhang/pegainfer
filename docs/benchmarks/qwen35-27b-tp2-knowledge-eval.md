...
| 基准 | 官方 | 实测 | n | 口径 | Δ 判定 |
|---|---|---|---|---|---|
| MMLU-Redux | 93.2 | 94.09(复跑待定,勿引用) | 5330 全量 | 8192 cap + 截断重跑合并(32 条重跑,2 条仍截断) | **无带内判定**:修复前抽取器打分 |
...
- **没有 TP 精度问题**(主证据:27B TP2 HF logits golden gate 全绿)。C-Eval 非截断子集(1290/1346)准确率 90.2% ≈ 官方 90.5;但截断与难度相关,该子集**不能证明**剩余差距全部来自 thinking 预算掐断——“差距源于截断难题而非模型错算”目前只是假设,需不限预算的同上下文对比证实。
