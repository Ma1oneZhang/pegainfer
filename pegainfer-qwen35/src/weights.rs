...
impl Qwen35Model {
    ...
    fn from_safetensors_with_runtime_and_capacity(
        ...
        let recurrent_reserve = STATES_PER_DECODE_SLOT
            * max_batch
            * super::recurrent_state::bytes_per_request(&config, tensor_parallel);
    ...
    pub(crate) fn tune_decode_gemm_algos(&self) -> Result<()> {
        ...
        let linear_qkv = self.config.local_linear_qkv_dim(tp);
        let linear_z = self.config.local_linear_z_dim(tp);
        let linear_ba = self.config.local_linear_num_value_heads(tp);
...
mod tests {
    ...
    fn linear_qkv_shard_segments_stitch_head_local_slices() {
        ...
        let config = test_config();
        let rank0 = linear_qkv_shard_segments(
            &config,
            TensorParallelConfig {
                rank: 0,
                world_size: 2,
        ...
        let rank1 = linear_qkv_shard_segments(
            &config,
            TensorParallelConfig {
                rank: 1,
                world_size: 2,
    ...
    fn linear_conv1d_shard_segments_scale_by_kernel_dim() {
        let config = test_config();
        let rank1 = linear_conv1d_shard_segments(
            &config,
            TensorParallelConfig {
                rank: 1,
                world_size: 2,
    ...
    fn linear_attention_tp2_slices_match_synthetic_checkpoint() {
        ...
        let config = test_config();
        let qkv_rows = config.local_linear_qkv_dim(TensorParallelConfig::default()); // 8192
        ...
        for rank in 0..2usize {
            ...
            assert_eq!(stitched.len(), config.local_linear_qkv_dim(tp) * cols);
            ...
            assert_eq!(segments[0].0, rank * config.local_linear_q_dim(tp));
            assert_eq!(
                segments[2].0,
                2 * global_q + rank * config.local_linear_v_dim(tp)
            );
            ...
            let lq = config.local_linear_q_dim(tp);
            let lk = config.local_linear_k_dim(tp);
            let lv = config.local_linear_v_dim(tp);
            ...
            assert_eq!(
                conv_stitched.len(),
                config.local_linear_qkv_dim(tp) * kernel_dim
            );
            ...
            assert_eq!(z_rows, config.local_linear_z_dim(tp));
        }
    }
}
